...
const TTS_LANGUAGE_INSTRUCTIONS = {
  ...
  da: "Read the text naturally in Danish.",
  ...
  hi: "Read the text naturally in Hindi.",
  bn: "Read the text naturally in Bengali.",
...
async function createLemonCheckout({ account, deviceToken, selectedPlan, returnUrl, state }) {
  ...
  // Hosted checkout links are generated by Lemon Squeezy in the dashboard and
  // remain valid even if its custom Checkout API relationship validation changes.
  // The custom fields are returned verbatim in the subscription webhook, which
  // lets us safely connect the payment to the signed-in extension account.
  const sharedCheckoutUrl =
    selectedPlan.id === "annual"
      ? LEMON_SQUEEZY_ANNUAL_CHECKOUT_URL
      : LEMON_SQUEEZY_MONTHLY_CHECKOUT_URL;
  if (sharedCheckoutUrl) {
    let url;
    try {
      url = new URL(sharedCheckoutUrl);
    } catch (_error) {
      throw new Error(`Hosted Lemon checkout URL is invalid for ${selectedPlan.id}.`);
    }
    if (url.protocol !== "https:" || !url.hostname.endsWith(".lemonsqueezy.com")) {
      throw new Error(`Hosted Lemon checkout URL is not a Lemon Squeezy URL for ${selectedPlan.id}.`);
    }
    url.searchParams.set("checkout[custom][accountId]", account.id);
    url.searchParams.set("checkout[custom][deviceToken]", deviceToken);
    url.searchParams.set("checkout[custom][planId]", selectedPlan.id);
    url.searchParams.set("checkout[custom][productSlug]", PRODUCT_SLUG);
    return {
      id: `lemon_hosted_${randomId("checkout")}`,
